...
+ `evidence` aggregates the most common audit commands into one report: `check`, `gen` summary, and `stress-plan` summary, plus the cptool version. Use `--json` for machine-readable reports; use `--skip-gen` or `--skip-stress-plan` when a package intentionally cannot run that section.
