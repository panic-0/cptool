...
cpp_compile_args: [-O2, -std=c++14, -I../../third_party/testlib/]
programs:
  gen:
    info: !cpp
      path: ./src/gen.cpp
...
Stress 计划可以省略 `generator`，此时使用顶层默认 generator。plan 参数支持 `{case}`（从 1 开始）和 `{case0}`（从 0 开始）。plan 默认 `expect: pass`；用 `expect: fail` 记录 wrong 程序证据。
