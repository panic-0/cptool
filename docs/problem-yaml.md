...
有 `score` 的 task 会落盘、导出并参与分值；没有 `score` 的 task 是 verify-only，只由 `test task` 运行，不进入正式数据。`pass` 中的程序必须匹配 `solution`，`fail` 中的程序至少要在该 task 的一个 case 上失败。注册在 `programs` 中但不是 solution/validator/checker/generator 的程序，必须出现在至少一个 `pass` 或 `fail` 中。
