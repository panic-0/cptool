...
声明了 `test.type` 时，有 `score` 的正式 task 可以省略 `type`。task 级 `type` 会覆盖该默认值。带 `pass`/`fail` 的 expect task 必须是 `min`；verify-only task 的 expect 语义固定按 `min` 处理，不能写 `type: sum`。

有 `score` 的 task 会落盘、导出并参与分值；没有 `score` 的 task 是 verify-only，只由 `test expect` 运行，不进入正式数据。`pass` 中的程序必须匹配 `solution`，`fail` 中的程序至少要在该 task 的一个 case 上失败；`test expect` 会按 task 内 case 顺序运行，`fail` 在首个有效失败后停止。注册在 `programs` 中但不是 solution/validator/checker/generator 的程序，必须出现在至少一个 `pass` 或 `fail` 中。
