...
输入 fixture 放在 `fixtures/input/`，用于正式 `:file` 数据 case。validator fixture 放在 `fixtures/validator/pass` 或 `fixtures/validator/fail`。checker fixture 放在 `fixtures/checker/pass/<name>/` 或 `fixtures/checker/fail/<name>/`，文件名固定为 `input.in`、`output.out` 和 `answer.ans`。
